...
public class ConfigurationPropertyAnnotationProcessor extends AbstractProcessor {
    ...
    private void resolveMetadata(RoundEnvironment roundEnv) {
        ...
        if (!elements.isEmpty()) {
            ...
            appendConfigurationPropertyGeneratedJSON(jsonBuilder);
    ...
    private void appendConfigurationPropertyGeneratedJSON(StringBuilder jsonBuilder) {
        try {
            ConfigurationPropertyGenerator.generateAll();
        } catch (Throwable e) {
            printMandatoryWarning(this.messager, "Failed to generate configuration property metadata JSON by ConfigurationPropertyGenerator : {}", e);
        }
    }
